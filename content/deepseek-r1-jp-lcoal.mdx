---
title: 日本語対応DeepSeek R1をArch Linux+RTX4070で動かしてみる
...
PythonやDocker等は入ってる前提で行きます。

## Nvidia関連のセットアップ

パッケージマネージャーはparuを使用してます。
...
```bash
paru -S nvidia nvidia-utils nvidia-container-toolkit opencl-nvidia
```

前にNvidia関連のものを入れたのを忘れている可能性があります。
もしかしたら全部必要ではないかもしれないです。また、足りないパッケージがある可能性があります。
...
llama.cppを直接ビルドしてもできると思いますが、最新のリポジトリで量子化のファイルが見つかりませんでした。
(ビルドしてもllama-quantizeファイルが生成されないぞ、、)  
知っている方いたら教えてください。  
参照: [https://github.com/ggerganov/llama.cpp/tree/master/examples/quantize](https://github.com/ggerganov/llama.cpp/tree/master/examples/quantize)

なのでllama-cliのllama-quantizeを使います。  
まずインストールします。

cudaバージョンがあったので、cudaバージョンをを入れておきます。
...
Modelfileを作成します。(なんかDockerfileみたいですね)  
...
Docker x Nvidia x WSL2(しかもArch Linux)の環境構築で骨が折れました。
...
[http://localhost:8080](http://localhost:8080)にアクセスして、画面通りにユーザー作るだけです。
...
ちなみに僕のM4 Mac Book Proはメモリが24GBなのでエラーでそもそも動かないです。
