...
description: あなたのNeovimライフがちょっとリッチになるプラグインをご紹介
date: 2024-12-10
---
...
本当は`get`で現在の状態を取得する必要があるのですが、showkeysのstateはexportされていないので、
参照できません。
そのため、自分で`state`変数を定義してそれを切り替えて表現しています。
...
ただし、コマンドで`ShowkeysToggle`を実行するとあべこべになってしまう点に注意が必要です。

気が向いたらstateをexportするようにPRを送ってみます。

また、このプラグインは`ShowkeysToggle`コマンドを叩くことで起動しますが、
それだとsnacks.nvimのトグルが利用できないので、`init`で`setup()`を実行して強制起動させます。
