...
export const SearchDialogHeader = (props: ComponentProps<'div'>) => {
  return (
    <div
      {...props}
...
export const SearchDialogInput = (props: ComponentProps<'input'>) => {
  const { text } = useI18n();
...
export const SearchDialogClose = ({
  children = 'ESC',
  className,
  ...props
...
export const SearchDialogFooter = (props: ComponentProps<'div'>) => {
  return (
    <div
      {...props}
...
export const SearchDialogOverlay = (
...
export const SearchDialogListItem = ({
  item,
  className,
  children,
      ...
      <>
        <div className='inline-flex items-center text-fd-muted-foreground text-xs empty:hidden'>
          {item.breadcrumbs?.map((item, i) => (
            // biome-ignore lint/suspicious/noArrayIndexKey: Breadcrumb positions are fixed within each search result.
            <Fragment key={i}>
...
export const SearchDialogIcon = (props: ComponentProps<'svg'>) => {
...
const renderHighlights = (
  ...
  return highlights.map((node, i) => {
    if (node.styles?.highlight) {
      return (
        // biome-ignore lint/suspicious/noArrayIndexKey: Highlight fragments are ordered text spans with no independent identity.
