...
import { useFooterItems } from '@/components/fumadocs/base/hooks/use-footer-items';
import { isActive } from '@/components/fumadocs/base/urls';
import { useTOCItems } from '@/components/fumadocs/components/toc';
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
...
import { useLayoutContext } from '@/components/fumadocs/layouts/docs/client';

const TocPopoverContext = createContext<{
  open: boolean;
  setOpen: (open: boolean) => void;
} | null>(null);
export const PageTOCPopover = ({
  className,
  children,
  ...rest
}: ComponentProps<'div'>) => {
...
export const PageTOCPopoverTrigger = ({
  className,
  ...props
}: ComponentProps<'button'>) => {
  const { text } = useI18n();
  const context = use(TocPopoverContext);
  if (!context) throw new Error('Missing PageTOCPopover.');
  const { open } = context;
...
export const PageTOCPopoverContent = (props: ComponentProps<'div'>) => {
  return (
    <CollapsibleContent
...
const FooterItem = ({ item, index }: { item: Item; index: 0 | 1 }) => {
  const { text } = useI18n();
